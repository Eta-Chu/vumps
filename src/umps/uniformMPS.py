...
class UniformMPS:
    ...
    def calTransMat(self, op=False, matrix=False):
        ...
        if op:
            ...
            E = ts.tensordot(self.A, self.A.conj(), ([1], [1]))
    ...
    def leftCanonical(self, iteration=True, tol=1e-14, L0=None, maxiter=None):
        if self.canonical_form != 'left':
            if iteration:
                ...
                val = ts.sqrt(val)
                self.L = ts.dot(vec * val, vec.conj().T)
                self.Linverse = ts.dot(vec * (1 / val), vec.conj().T)
    ...
    def rightCanonical(self, iteration=True, tol=1e-14, R0=None, maxiter=None):
        if self.canonical_form != 'right':
            if iteration:
                ...
                val = ts.sqrt(val)
                self.R = ts.dot(vec * val, vec.conj().T)
                self.Rinverse = ts.dot(vec * (1 / val), vec.conj().T)
    ...
    def mixCanonical(self, iteration=True, tol=1e-14, V0=None, maxiter=None):
        if self.canonical_form != 'mix':
            self.rightCanonical(iteration, tol, V0, maxiter)
            self.leftCanonical(iteration, tol, V0, maxiter)
            self.C = ts.dot(self.L, self.R)
            U, self.C, Vh = ts.linalg.svd(self.C)
            self.C /= ts.sqrt(self.C @ self.C.conj())
            self.Al = ts.tensordot(U.conj().T, self.Al, ([1], [0]))
    ...
    @staticmethod
    def random(D: int, d: int) -> "UniformMPS":
